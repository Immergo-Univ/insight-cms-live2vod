/**
 * Frame-level video metrics computed locally (no ML): energy, motion, scene changes,
 * black-screen ratio and dominant-color change. Frames are downscaled to a tiny grayscale
 * (and RGB) thumbnail so diffs are cheap and robust to codec noise.
 */
...
export async function computeFrameMetrics(framePaths) {
  ...
  // Energy = variability of motion across the window (how "busy"/changing it is).
  // Combine mean motion with its own spread so a fixed slate (low motion, low spread) scores near 0.
  const motionMean = motion_avg;
  const motionVar = diffs.length
    ? diffs.reduce((s, x) => s + (x - motionMean) ** 2, 0) / diffs.length
    : 0;
  const energy_avg = Math.min(1, motionMean + Math.sqrt(motionVar));
...
export default { computeFrameMetrics };
