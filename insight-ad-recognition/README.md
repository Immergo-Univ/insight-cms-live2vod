...
- **CLAP** (clasificador de audio zero-shot): categorías de contenido + música/locución.
...
```mermaid
flowchart TB
  In["GET /detect?video=&startTime=&endTime="] --> M["media.service: ffmpeg ultimos ~10s @1fps + audio 48k/16k"]
  ...
  M --> AUD["sidecar /audio: CLAP chunks de 5s"]
  GATE & A & W & VIS & CUES & TXT & AUD --> FUSE["fusion.service: score multimodal + consistencia temporal intra-ventana"]
  FUSE --> Out["detection ad/program/silence + score + confidence + profile"]
```

1. **Extracción**: `ffmpeg` saca ~10 frames (1 fps) + audio 48 kHz (CLAP) + 16 kHz (whisper).
...
5. **Audio**: CLAP por chunks + métricas locales.
...
Ver `.env.example`. Principales: `SEGMENT_SECONDS` (10), `HEAVY_MAX_FRAMES` (5), `AUDIO_CHUNK_SECONDS` (5), `SIGLIP_MODEL`, `TEXT_MODEL`, `CLAP_MODEL`, `OCR_LANGUAGES` (`heb+eng+spa`), `AD_MIN_SCORE` (0.35), `AD_CATEGORIES`, y los pesos `FUSION_*`.
