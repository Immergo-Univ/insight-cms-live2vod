...
function specWithTiktokUploadPatch(spec, editorClipId, accountId, uploadPatch) {
  ...
  const prevUploads =
    tt.uploads && typeof tt.uploads === "object" && !Array.isArray(tt.uploads) ? { ...tt.uploads } : {};
  const prevAccountUp =
  ...
  tt.uploads = prevUploads;
  tt.upload = prevUploads[accountId];
  synd.tiktok = tt;
...
export async function tryTiktokSyndicationAfterJobCompleted(jobId) {
  ...
  if (!tenantRow) return;
  if (tenantRow.get({ plain: true }).syndicationTiktokEnabled !== true) return;

  const accounts = await getActiveAccountsForPublish(job.tenantId, "tiktok");
  ...
  for (const account of accounts) {
    ...
    const prevUp =
      uploadsMap[accountId] && typeof uploadsMap[accountId] === "object" && !Array.isArray(uploadsMap[accountId])
        ...
        : tt.upload && typeof tt.upload === "object" && !Array.isArray(tt.upload) && accounts.length === 1
          ? tt.upload
          : {};
    if (prevUp.state === "published" || prevUp.state === "uploading") continue;

    await mergeJobEditorSpec(jobId, (prev) =>
      specWithTiktokUploadPatch(prev, job.editorClipId || "", accountId, {
        state: "uploading",
    ...
    try {
      ...
      vodEncodeStdout(`tiktok-syndication ok job=${jobId} account=${accountId} publishId=${result.publishId}`);
    } catch (e) {
      const m = e instanceof Error ? e.message : String(e);
      vodEncodeStdout(`tiktok-syndication failed job=${jobId} account=${accountId} err=${m.slice(0, 400)}`);
      await mergeJobEditorSpec(jobId, (prev) =>
        specWithTiktokUploadPatch(prev, job.editorClipId || "", accountId, {
          state: "failed",
